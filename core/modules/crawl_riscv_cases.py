...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Crawl syzbot cases and keep RISCV/RISCV64 cases only."
    )
    parser.add_argument(
        "--url",
        default="https://syzkaller.appspot.com/upstream/fixed?manager=ci-qemu2-riscv64",
        help="Syzbot list URL (default: riscv64 manager filtered).",
    )
    parser.add_argument(
        "--max",
        type=int,
    ...
    parser.add_argument(
        "--arch-regex",
        default=r"(riscv64|\briscv\b)",
        help="Regex used to match manager field.",
    )
    parser.add_argument(
        "--output",
        default="work/Syzbot_RISCV_cases_get-basic-info.json",
    ...
    parser.add_argument(
        "--save-all",
        action="store_true",
        help="Also save all crawled cases before RISCV filtering.",
    )
    parser.add_argument(
        "--all-output",
...
def main():
    ...
    # RISCV64 cases use riscv/linux.git (non-upstream kernel repo),
    # so we force --allcase to skip the "upstream" kernel check.
    # The manager-based arch filtering still applies for accuracy.
    effective_allcase = args.allcase or True
    ...
    crawler = Crawler(
        ...
        debug=args.debug,
        arch_regex=args.arch_regex,
    )

    # Need detail mode so each case has manager field for architecture filtering.
    crawler.run(OnlyTitle=False, AllCase=effective_allcase)
    ...
    for case_hash, case_info in all_cases.items():
        ...
        if arch_re.search(manager):
            if not args.allow_incomplete and not is_complete_case(case_info):
                incomplete_count += 1
                continue
            riscv_cases[case_hash] = case_info

    output_path = resolve_output_path(args.output)
    save_json(output_path, riscv_cases)
    ...
    print(f"[*] RISCV matched cases: {len(riscv_cases)}")
    if not args.allow_incomplete:
        print(f"[*] RISCV incomplete cases skipped: {incomplete_count}")
    print(f"[*] RISCV cases saved to: {output_path}")
